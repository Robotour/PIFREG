#!/usr/bin/env python3
"""
PIFReg 联合栈配准实验（单次优化，共享位移场）

与 run_groupwise_registration_exp.py 评价/输出格式一致；
与 run_pifreg_groupwise_exp.py（pairwise chain/mean）不同，本脚本仅调用
register_pifreg_groupwise_joint —— 全栈一次优化，速度远快于 N 次 pairwise PIFReg。
"""
...
def save_rgb_comparison(rgb_before, rgb_after, output_path):
    ...
    axes[1].set_title("RGB After PIFReg Joint Groupwise")
    axes[1].axis("off")
    plt.tight_layout()
...
def run_experiment(
    ...
    print("=" * 60)
    print("Single shared-flow optimization on (N,H,W) stack")
    print("(Elastix-like spectral variance + one U-Net, NOT N× pairwise)")
    print("-" * 60)
    print(f"Input folder : {stack_dir}")
    print(f"Output folder: {output_dir}")
    print(f"Device       : {device}")
    print(f"Image size   : {image_size}")
    print(f"epochs       : {epochs}, fast_mode={fast_mode}")
    ...
    print("[2/4] Running joint PIFReg groupwise (single optimization) ...")
    t0 = time.perf_counter()
    bands_after, reg_info = register_pifreg_groupwise_joint(
        bands_before,
        device=str(device),
        epochs=epochs,
        fast_mode=fast_mode,
...
def parse_args():
    ...
    p.add_argument("--ref-band", type=int, default=None, help="U-Net 输入参考波段")
    p.add_argument("--eval-ref-band", type=int, default=None, help="评估用参考波段")
